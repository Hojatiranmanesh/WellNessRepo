...
const sendNotif = (data) => {
    ...
    if (Notification.permission === 'granted') {
        ...
        const notification = new Notification("ایران‌ولنس", {
            ...
            icon: `https://api.hamyarwellness.com/${data.image}`,
            image: `https://api.hamyarwellness.com/${data.image}`,
        });
...
const Cart = () => {
    ...
    useEffect(() => {
        axios.get('https://api.hamyarwellness.com/api/v1/products/cart', { headers: { 'Authorization': token } },)
            .then(res => {
            ...
            <Box className={classes.cartContainer}>
                {cartItems.map(item => (
                    <Box className={classes.cartItem}>
                        {/* <img className={classes.productImage} src={`https://api.hamyarwellness.com/${item.product.image}`} alt="" /> */}
