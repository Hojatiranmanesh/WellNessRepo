...
const SearchUserInfo = () => {
    ...
    const searchUsers = () => {
        axios.get(`https://api.hamyarwellness.com/api/v1/users/search?${searchFirstname ? 'firstname=' + searchFirstname : ""}${searchLastname ? '&lastname=' + searchLastname : ""}${searchPhone ? '&phone=' + searchPhone : ""} `, { headers: { 'Authorization': `bearer ${localStorage.getItem('jwt')} ` } })
            .then(res => {
    ...
    const showModal = e => {
        console.log(e)
        handleOpen()
        axios.get(`https://api.hamyarwellness.com/api/v1/users/search?_id=${e}`, { headers: { 'Authorization': `bearer ${localStorage.getItem('jwt')}` } })
            .then(res => {
    ...
    const updateUser = () => {
        ...
        axios.put(`https://api.hamyarwellness.com/api/v1/users/${modalId}`, data, header)
            .then(res => {
                console.log(res)
                alert('ثبت شد')
            }).catch(err => {
